class ProjectCasesController < ApplicationController
  ...
  def project_case_params
    params.require(:project_case).permit(:project_case_name, :prefecture_id, :project_address_city, :project_address_address, :project_case_pr,:image).merge(user_id: current_user.id)
  end
end
